...
云笺集是基于 RAGNotebook 二次开发的智能笔记与知识库系统。项目围绕“资料沉淀、智能检索、持续复习、结构化输出”构建学习闭环，将原有 RAG Notebook 思路升级为一套可本地部署、可持续维护、具备用户隔离和数据库迁移能力的工程化应用。

当前项目采用前后端分离架构：

| 层级 | 当前实现 |
| --- | --- |
| 前端 | Vue3、TypeScript、Vite、Pinia、Vue Router、Tailwind CSS、Tiptap、Vue Flow |
...
本项目不是单一问答 Demo，而是面向个人学习、资料管理和知识复用的智能工作台，主要能力包括笔记管理、知识库上传、RAG 问答、AI 写作辅助、间隔重复回顾、快速测试和思维导图生成。
...
| 笔记复用率低 | 记录完成后缺少回顾、测验和结构化整理机制 |
| 知识检索效率低 | 关键词搜索无法理解语义，跨文档查找成本高 |
| 学习效果难验证 | 学完资料后缺少自动出题、反馈和薄弱点总结 |
| 工程可维护性不足 | 缺少统一数据库、迁移、测试、启动脚本和接口约定 |

因此，本项目需要建设一套具备企业级工程基础的智能知识管理系统，使用户能够把资料、笔记、问答、复习和测验统一纳入同一个应用闭环。
...
| 学习目标 | 通过回顾、快速测试和结构化导图提高知识复用率 |
| 数据目标 | 使用 PostgreSQL 统一承载关系数据、向量数据和运行态数据 |
| 工程目标 | 建立可维护的前后端分离架构、数据库迁移、统一配置和测试体系 |
...
| 用户角色 | 使用场景 | 主要诉求 |
| --- | --- | --- |
| 普通学习用户 | 写笔记、上传资料、问答、复习、测验 | 降低资料整理成本，提高学习效率 |
| 知识管理用户 | 建立个人知识库，按主题查询和复用资料 | 统一管理、多源检索、持续积累 |
| 系统管理员 / 开发维护者 | 部署服务、维护数据库、排查模型和依赖问题 | 可配置、可迁移、可诊断、可扩展 |
...
| FR-06 | 间隔重复回顾 | 中 | 后端 `/review`，记录回顾次数和下次回顾时间 |
| FR-07 | NotebookLM 风格快速测试 | 高 | 后端 `/quick-test`，支持创建、答题、结束和总结 |
| FR-08 | 思维导图生成、编辑、查询和导出 | 高 | 后端 `/mindmaps`，前端 Vue Flow 渲染 |
| FR-09 | 会话历史管理 | 中 | PostgreSQL 存储聊天会话和消息 |
| FR-10 | 系统健康检查和后台初始化状态 | 高 | 后端 `/health`，覆盖数据库和模型运行态 |
...
| 可扩展性 | 后续可新增接口、文档类型和 Agent 工具 | 通过 schemas、services、routers、Alembic 和配置文件扩展 |
| 数据一致性 | 表结构可追踪，避免运行时隐式补表 | 使用 Alembic 管理 PostgreSQL 和 pgvector 初始化 |
...
| 目录 | 职责 |
| --- | --- |
| `views` | 登录、笔记、知识库、聊天、回顾、快速测试、思维导图等页面 |
...
回顾模块按照间隔重复思路记录用户需要复习的笔记，帮助用户持续使用历史内容。
...
思维导图模块将笔记或知识库内容结构化为 nodes / edges 图数据，前端用 Vue Flow 渲染。用户可查询、更新图结构，并导出 JSON 或 Mermaid。
...
项目使用 PostgreSQL 作为统一数据底座，使用 pgvector 承载知识库和笔记向量，使用 Alembic 管理表结构。
...
| 数据库结构漂移 | 通过 Alembic revision 记录结构变更 |
...
| 后端开发主力 | 1 | FastAPI 接口、PostgreSQL、Alembic、权限、会话、运行态治理、接口联调 | 后端接口、数据库迁移、OpenAPI、后端测试 |
...
| 第 2 阶段：基础架构与数据库建设 | 建立可运行工程底座 | FastAPI、Vue3、PostgreSQL、pgvector、Alembic、启动脚本、登录鉴权 | 可启动环境、数据库迁移、基础接口 |
| 第 3 阶段：核心业务模块开发 | 完成主要业务闭环 | 笔记、知识库、聊天问答、回顾、会话管理、前后端联调 | 核心功能可演示 |
...
测试目标是确认系统满足需求分析中的功能性和非功能性要求，重点验证核心链路、用户隔离、数据库迁移、AI 模块可用性和前后端联调稳定性。
...
| 接口测试 | 登录、笔记、知识库、聊天、回顾、快速测试、思维导图 | 测试工程师 + 后端开发 |
| 前端功能测试 | 页面路由、表单、列表、编辑器、流式输出、导图交互 | 测试工程师 + 前端开发 |
| 数据库测试 | Alembic 迁移、pgvector 表、运行态表、用户数据隔离 | 后端开发 + 测试工程师 |
...
| 关注点 | 处理方式 |
| --- | --- |
| 数据库迁移 | 使用 Alembic revision 管理，空库由后端启动初始化 |
...
| pgvector 未安装 | 数据库迁移或向量检索失败 | 本地使用 `pgvector/pgvector:pg16`，外部库需先安装扩展 |
...
| AC-06 | 回顾功能 | 可查看待回顾内容并标记完成 |
| AC-07 | 快速测试 | 可创建测试、答题、获得反馈并生成结束总结 |
| AC-08 | 思维导图 | 可生成、查看、编辑并导出导图 |
| AC-09 | 健康检查 | `/health` 能反映数据库和后台初始化状态 |
| AC-10 | 用户隔离 | 不同用户之间不能读取彼此数据 |
...
云笺集当前已经具备较完整的企业级开发基础：前后端分离、统一 PostgreSQL 数据底座、pgvector 向量检索、Alembic 迁移、JWT 鉴权、运行态治理、一键启动脚本和测试基础。面向 5 人团队协作时，可由项目经理统筹需求和验收，3 名开发主力分别负责后端、前端和 AI / 数据链路，测试工程师负责质量闭环。
