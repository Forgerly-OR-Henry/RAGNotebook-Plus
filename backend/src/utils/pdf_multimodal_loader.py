...
async def pdf_multimodal_loader(file_path: str, content_hash: str, user_id: str) -> list[Document]:
    ...
    设计决策：
    - 视觉补充：纯文本提取无法覆盖图表、流程图、表格结构等视觉信息，
      视觉模型用于补全页面图像语义，提升 RAG 检索的召回质量。
    - 相似页去重：PDF 中可能包含重复的装饰性图片（如页眉页脚、背景图），
      通过感知哈希合并相似页面，减少重复视觉模型调用。
    - 批量处理：视觉模型 API 通常支持批量图片输入，按批提交可减少请求次数并提高吞吐量。
    """
