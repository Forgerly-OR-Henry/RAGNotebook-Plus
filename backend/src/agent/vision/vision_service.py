...
class VisionService:
    """
    多模态视觉服务，用于将页面图片提交给视觉模型并生成结构化文本描述。

    设计说明：
    传统 PDF 文本解析无法完整覆盖图片、图表、流程图和版式结构等视觉信息。
    本服务通过视觉大模型（如 Qwen-VL）对 PDF 页面截图进行语义描述，
    将视觉内容转换为可检索文本并补充到 Document 内容中，从而提升 RAG 召回质量。
    """
    ...
    def _dashscope_describe(self, img_b64: str, mime: str, existing_text: str) -> str:
        """
        用途：执行dashscope describe相关业务逻辑。

        参数：
        - img_b64（str）：调用方传入的img_b64数据或控制参数，用于驱动本函数处理流程。
        - mime（str）：调用方传入的mime数据或控制参数，用于驱动本函数处理流程。
        - existing_text（str）：调用方传入的existing_text数据或控制参数，用于驱动本函数处理流程。
    ...
    def _dashscope_describe_batch(
        ...
        """
        用途：执行dashscope describe batch相关业务逻辑。

        参数：
        - images_info（list[tuple[str, str, str]]）：调用方传入的images_info数据或控制参数，用于驱动本函数处理流程。
        - page_numbers（list[int]）：调用方传入的page_numbers数据或控制参数，用于驱动本函数处理流程。
