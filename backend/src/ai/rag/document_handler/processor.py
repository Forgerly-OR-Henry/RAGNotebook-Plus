...
from ai.rag.text_spliter import AsyncTextSplitter
from utils.config import vector_config
...
class DocumentProcessor:
    ...
    def __init__(self, vector_store, md5_store, embed_model=None):
    ...
    async def get_file_document(
        ...
        if lower_path.endswith('.txt'):
            ...
            if use_multimodal and md5 and user_id:
    ...
    async def get_document(self, files: list = None, user_id: str = None, progress_callback=None):
        ...
        if files:
            ...
            allowed_file_path: tuple[str] = await listdir_allowed_type(
                vector_config['data_path'],
                tuple(vector_config['allow_knowledge_file_types'])
            )
        ...
        for idx, file_path in enumerate(file_paths):
            ...
            try:
                ...
                # 上传阶段只做文本解析和向量写入，不调用视觉识别模型；
                # 后续 AI 问答等调用方如需多模态解析，可显式启用 use_multimodal。
                document: list[Document] = await self.get_file_document(
                    file_path,
                    md5_hex,
                    user_id,
                    use_multimodal=not files,
                )
                if not document:
                    if progress_callback:
                        await progress_callback({
                ...
                await self.vector_store.add_documents(document, user_id=user_id)
