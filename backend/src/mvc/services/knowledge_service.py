...
class KnowledgeService:
    ...
    async def list_documents(self, db: AsyncSession, user_id: str) -> list[dict]:
        ...
        return [self._document_to_dict(document, storage_object) for document, storage_object in result.all()]
    ...
    async def get_document_detail(self, db: AsyncSession, user_id: str, document_id: str) -> dict | None:
        ...
        return {
            **self._document_to_dict(document, storage_object),
    ...
    async def clean_user_documents(self, db: AsyncSession, user_id: str) -> None:
        ...
        for document, storage_object in result.all():
            await self.index_repository.delete_source(user_id=user_id, source_type="knowledge", source_id=document.id)
            try:
                await self.storage_service.delete_storage_object_data(storage_object)
            except Exception as exc:
    ...
    async def _get_document_row(self, db: AsyncSession, user_id: str, document_id: str):
        result = await db.execute(
            select(Document, StorageObject)
            .join(StorageObject, Document.storage_object_id == StorageObject.id)
            .where(Document.id == document_id, Document.user_id == user_id, Document.source_type == "knowledge")
        )
        return result.one_or_none()

    @staticmethod
    def _document_to_dict(document: Document, storage_object: StorageObject) -> dict:
        return {
            "id": document.id,
            "document_id": document.id,
